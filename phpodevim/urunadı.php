<?php
session_start(); // Oturumu başlatır, böylece $_SESSION süper küresel değişkeni kullanılabilir.

// Sepet içeriğini kontrol etme ve gerekirse oturumu sıfırlama
if (!isset($_SESSION['initialized'])) {
    $_SESSION['initialized'] = true; // Oturumun başlatıldığını işaretler
    $_SESSION['cart'] = array(); // Sepet içeriğini başlatır
}
...
function displayCart() {
    ...
    if(isset($_SESSION['cart'])) { // Sepet varsa
        foreach ($_SESSION['cart'] as $key => $product) { // Her ürün için döngü
            ...
            echo "<form method='post'>"; // Ürün çıkarma formu
            echo "<input type='hidden' name='index' value='$key'>"; // Ürünün indexini gizli alanla gönderir
    ...
    $totalPrice = 0;
    if(isset($_SESSION['cart'])) { // Sepet varsa
        foreach ($_SESSION['cart'] as $product) { // Her ürün için döngü
            $totalPrice += floatval(str_replace(',', '.', $product['price'])); // Fiyatı float olarak al ve topla
        }
    }
    echo "<div class='max-w-md mx-auto p-4 border bg-gray-800 border-gray-200 rounded-lg mt-4'>";
    echo "<p class='text-lg font-semibold text-white'>Sepet Toplamı: " . number_format($totalPrice, 2, ',', '.') . " ₺</p>"; // Toplam fiyatı göster
    echo "</div>";
    echo "</div>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['product'])) {
    ...
    $product = json_decode($_POST['product'], true); // JSON verisini diziye dönüştür
    
    // Sepete ekle
    if(!in_array($product, $_SESSION['cart'])) { // Ürün daha önce eklenmemişse
        array_push($_SESSION['cart'], $product); // Ürünü sepete ekle
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['removeProduct']) && isset($_POST['index'])) {
    ...
    unset($_SESSION['cart'][$index]); // Belirtilen index'teki ürünü kaldır
}
...
<!-- Sepet -->
<div>
    <?php displayCart(); // Sepet içeriğini gösteren fonksiyonu çağır ?>
</div>
<script>
